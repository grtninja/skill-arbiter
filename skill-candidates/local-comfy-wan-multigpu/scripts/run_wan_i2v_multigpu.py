#!/usr/bin/env python3
"""Run a local Wan image-to-video render with MultiGPU loader placement."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Wan image-to-video with MultiGPU nodes")
    parser.add_argument("--base-url", default="http://127.0.0.1:8000", help="Comfy base URL")
    parser.add_argument("--input-image", required=True, help="Image filename as seen by LoadImage")
    parser.add_argument("--width", type=int, default=720, help="Target width")
    parser.add_argument("--height", type=int, default=1280, help="Target height")
    parser.add_argument("--length", type=int, default=161, help="Wan latent length")
    ...
    parser.add_argument("--filename-prefix", default=default_filename_prefix("wan_render"), help="Comfy filename prefix")
    parser.add_argument("--out-dir", type=Path, required=True, help="Evidence/output folder")
    parser.add_argument("--run-name", default="wan_render", help="Base filename for JSON artifacts")
...
def build_prompt(args: argparse.Namespace) -> dict[str, dict]:
    return {
        "9000": {"class_type": "LoadImage", "inputs": {"image": args.input_image}},
        "84": {"class_type": "CLIPLoaderMultiGPU", "inputs": {"clip_name": "umt5_xxl_fp8_e4m3fn_scaled.safetensors", "type": "wan", "device": "cpu"}},
        "90": {"class_type": "VAELoaderMultiGPU", "inputs": {"vae_name": "wan_2.1_vae.safetensors", "device": "cuda:0"}},
        "95": {"class_type": "UNETLoaderMultiGPU", "inputs": {"unet_name": "wan2.2_i2v_high_noise_14B_fp8_scaled.safetensors", "weight_dtype": "default", "device": "cuda:1"}},
        "96": {"class_type": "UNETLoaderMultiGPU", "inputs": {"unet_name": "wan2.2_i2v_low_noise_14B_fp8_scaled.safetensors", "weight_dtype": "default", "device": "cuda:0"}},
...
def main() -> int:
    ...
    # Fail fast if Comfy is not reachable or the input image is not registered.
    ...
    if args.input_image not in options:
        error_path.write_text(json.dumps({"error": "input_image_not_visible_to_loadimage", "input_image": args.input_image}, indent=2), encoding="utf-8")
        return 1
