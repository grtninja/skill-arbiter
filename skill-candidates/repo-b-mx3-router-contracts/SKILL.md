...
description: Validate MX3 shim model-router network-profile contracts and policy behavior. Use when changing route-mode derivation, router environment mapping, capability metadata, or inference routing responses.
---

# Repo B MX3 Router Contracts

Use this skill for router-profile-aware model routing checks in the MX3 shim lane.
...
2. Set router profile env vars for deterministic scenario testing:
   ...
   - `MESHGPT_ROUTER_MODE`
   - `ROUTER_NETWORK_MODE`
3. Run router-focused tests after every behavior change.
4. Validate that health and inference responses expose effective routing metadata.
